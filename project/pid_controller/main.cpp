  ...
  **/
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          // find the index of the point closest to the robot
          double min_distance = std::numeric_limits<double>::max();
          int closest_id = 0;
          for (int i = 0; i < x_points.size(); i++)
          {
            double distance = std::pow((x_position - x_points[i]),2) + std::pow((y_position - y_points[i]),2);
          ...
          std::cout << "Closest point id: " << closest_id << std::endl;
          ...
          **/
          ...
          error_throttle = v_points[closest_id] - velocity;
